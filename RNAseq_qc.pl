...
$Flat_ref = "/Users/kingdavid/Desktop/stanford/RNA_seq/ref/refFlat.txt";
...
while(<IN>){
  ...
  chdir $workdir;
  print "$id\n";
  $cmd2 = "java -Xmx2g -jar $picard_dir SortSam INPUT=Aligned.out.sam OUTPUT=$id.sorted.bam SORT_ORDER=coordinate 2> picard.log";
  $cmd3 = "java -Xmx2g -jar $picard_dir CollectRnaSeqMetrics STRAND_SPECIFICITY=NONE VALIDATION_STRINGENCY=SILENT REF_FLAT=$Flat_ref RIBOSOMAL_INTERVALS=$curdir/rRNA.interval.txt CHART_OUTPUT=$id.coverage.pdf INPUT=$id.sorted.bam OUTPUT=$id.rnaseqmetrics.dat 2>> picard.log";
  $cmd4 = "java -jar $picard_dir AddOrReplaceReadGroups I=$id.sorted.bam O=$id\_added_sorted.bam SO=coordinate RGID=$id RGLB=$id RGPL=hiseq RGPU=hiseq RGSM=$id 2>> picard.log";
  $cmd5 = "java -Xmx2g -jar $picard_dir MarkDuplicates I=$id\_added_sorted.bam O=$id\_dedupped.bam READ_NAME_REGEX='[a-zA-Z0-9]+.*:[0-9]:([0-9]+):([0-9]+):([0-9]+)\$' CREATE_INDEX=true VALIDATION_STRINGENCY=LENIENT M=$id.dupmetrics.dat 2>> picard.log";
  system("$cmd2");
  system("$cmd3");
  system("$cmd4");
  system("$cmd5");
